            ...
            if (dataGridView != null && e.RowIndex > -1 && e.ColumnIndex > -1)
            {
                ...
                if (selectedRow != null && selectedColumn != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(no))
                    {
                        ...
                        var result = APIHelper.Post<ApiResult<Student>>(WebSetting.GetUrl("Student/Single"),new Dictionary<string, object> {{ "Id",no}});
                        if (result.Code == 0)
                            student = result.Data;
                        else
                            MessageBox.Show(result.Msg);
                        ...
                        else
                        {
                            switch (selectedColumn.Name)
                            {
                                case "CheckOut":
                                    ...
                                    bool success = false;
                                    var checkOutResult = APIHelper.Post<ApiResult<Student>>(WebSetting.GetUrl("Student/CheckOut"), new Dictionary<string, object> { { "Id", no } });
